...
export default async function globalSetup() {
  ...
  try {
    ...
    throw new Error(
      "E2E tests require a working Docker/OCI runtime. Start Docker (or Podman) and rerun `npm run test:e2e --workspace=backend`.",
      { cause: error as Error }
    );
  }

  const databaseUrl = `postgresql://postgres:postgres@${postgres.getHost()}:${postgres.getMappedPort(5432)}/cine_manager_e2e?schema=public`;
